...
if test -f /root/jvscripts/log.txt
	then
		rm /root/jvscripts/log.txt
fi

if test -f /root/jvscripts/log2.txt
	then
		rm /root/jvscripts/log2.txt
fi
...
while [ $opcion = 0 ] 
do
	echo `netstat -plan | grep "ESTABLECIDO" |grep -e ":80" -e ":443" -e ":21" | awk {'print $5'} | uniq > /root/jvscripts/log2.txt`
	contador2=`wc -l /root/jvscripts/log2.txt | cut -d " " -f1`
		...
		then
			if [ $contador -lt $contador2 ]
				then
					...
					conexion=`tail -n 1 /root/jvscripts/log.txt`
